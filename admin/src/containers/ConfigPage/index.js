 ...
 *
 * ConfigPage
 * 配置页面默认不可以从插件列表中点击设置按钮访问
 * 可以修改 admin组件
 * /strapi-admin/admin/src/components/Row/index.js -> admin/src/components/Row/index.js
 * 并修改 PLUGINS_WITH_CONFIG 增加当前组件名称
 *
 */
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.submitSuccess !== this.props.submitSuccess) {
      this.props.history.push(
        `/plugins/${pluginId}/configurations/${this.props.match.params.env}`
      );
    }
  }
    ...
    const headerNavLinks = this.props.appEnvironments
      .reduce((acc, current) => {
        const link = Object.assign(current, {
          to: `/plugins/${pluginId}/configurations/${current.name}`,
        });
        acc.push(link);
          ...
          <ContainerFluid>
            <Header
              ...
              content={formatMessage({
                id: 'ctrip-apollo.ConfigPage.description',
              })}
              title={{ label: formatMessage({ id: 'ctrip-apollo.ConfigPage.title' }) }}
            />
            <HeaderNav
